...
- `webfetch`: default 50k chars output, hard cap 200k. 5 MB response cap. 30s timeout. **Cannot fetch:** images, video, audio, localhost, 127/8, 169.254/16; PDFs unless optional `pdftotext` is installed (see [PDF support](#pdf-support-optional)). **Cannot render:** JS-heavy SPAs (you'll get an empty markdown).
