...
- `webfetch` pagination: pass `offset` (default 0) to read past the 200k-char per-call cap. When the previous call returned a `[TRUNCATED — returned chars [X, Y) of Z total. Re-call with offset=Y to read the next chunk.]` footer, thread that `Y` back as `offset`. Range is half-open: `X` inclusive, `Y` exclusive (matches `String.prototype.slice`), so the chunks tile cleanly. No cache between calls — each paginated read re-fetches **and** re-runs the extractor pipeline (pandoc/trafilatura subprocess spawn — often the slower half on chrome-heavy pages; for PDFs, `pdftotext` is re-spawned on every call, so a 2 MB PDF paginated 3× = 3 full PDF extractions). Paginating a 600k-char doc therefore costs three full fetches and three full extractions. For large PDFs and HTML pages prefer one large `max_chars` over multiple paginated reads. Reconstructed output may also desync across chunks if the upstream page changes mid-pagination, or if a non-deterministic extractor (e.g. trafilatura version drift, dynamic content) yields different boundaries on a re-run. JSON responses are wrapped in ```` ```json … ``` ```` fences before pagination, so any chunk past the first slices into the body without an opening fence — in practice JSON responses fit in one chunk and this is rarely observed. Suggested workflow for long docs: stop paginating once the section the agent needs is in hand; don't reconstruct the whole document unless required.
